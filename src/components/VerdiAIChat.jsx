      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            className="fixed bottom-6 right-6 z-50 group flex items-center gap-2.5 pl-4 pr-5 py-3.5 rounded-full bg-[#B08A4C] text-white shadow-[0_8px_32px_rgba(139,111,61,0.25)] hover:shadow-[0_12px_40px_rgba(139,111,61,0.4)] transition-shadow"
          >
            {/* Pulsing ring */}
            <span className="absolute inset-0 rounded-full bg-[#B08A4C] animate-ping opacity-20 pointer-events-none" />
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 bg-[#0a0e17] space-y-4">
              <AnimatePresence initial={false}>
                {messages.map((msg) => (
                  ...
                  >
                    {msg.role === 'assistant' && (
                      <div className="w-7 h-7 rounded-full bg-[#B08A4C]/10 border border-[#B08A4C]/20 flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div
                      className={`max-w-[78%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${
                        msg.role === 'user'
                          ? 'bg-[#B08A4C] text-white rounded-tr-sm'
                ...
                <div className="flex flex-wrap gap-2 pt-2 pl-9 pb-2">
                  {(SUGGESTIONS[language] ?? SUGGESTIONS.tr).map((sug, idx) => (
                    <motion.button
                      ...
                      whileHover={{ scale: 1.05, backgroundColor: 'rgba(139, 111, 61, 0.2)', borderColor: '#B08A4C' }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => submitMessage(sug.text)}
                      className="text-[11px] font-medium px-3 py-2 rounded-full border border-[#B08A4C]/40 bg-[#B08A4C]/5 text-[#D9B97A] hover:text-white transition-colors text-left shrink-0"
                    >
                      {sug.label}
                    </motion.button>
                  ))}
              ...
              <AnimatePresence>
                {loading && (
                  ...
                  >
                    <div className="w-7 h-7 rounded-full bg-[#B08A4C]/10 border border-[#B08A4C]/20 flex items-center justify-center shrink-0">
                      <Sparkles className="w-3 h-3 text-[#D9B97A]" />
                    </div>
            ...
            <form onSubmit={sendMessage} className="flex items-end gap-2 px-3 py-3 bg-[#0a0e17] border-t border-white/10 shrink-0">
              <input
                ...
                className="flex-1 bg-[#131924] border border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#B08A4C]/50 resize-none"
              />
              <motion.button
                type="submit"
                ...
                whileTap={{ scale: 0.93 }}
                className="w-10 h-10 rounded-xl bg-[#B08A4C] text-white flex items-center justify-center shrink-0 disabled:opacity-40 transition-opacity"
              >
