...
const SUB_TITLE_CLS = 'font-fraunces text-3xl font-semibold leading-tight text-[#0A1428] md:text-4xl';
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#F1EEE7] text-[#0A1428]">
      ...
      <main>
        {/* ── Hero (stack layer 1) ── */}
        <section id="home" data-nav-theme="dark" className="sticky top-0 z-10 min-h-[760px] overflow-hidden bg-[#08132A] text-white lg:h-screen lg:min-h-screen">
          <img
            ...
            <div className="mb-24 grid w-full max-w-[1680px] gap-8 lg:mb-28 lg:ml-4 lg:grid-cols-[minmax(0,1180px)_260px] lg:items-end lg:gap-16">
              ...
              <Enter delay={0.55} from="right">
                <motion.a
                  ...
                  href="/iletisim"
                  className="inline-flex h-16 w-full max-w-[220px] items-center justify-center justify-self-start border border-white/70 px-8 text-sm font-bold uppercase tracking-[0.16em] text-white transition-colors hover:bg-white hover:text-[#0A1428] lg:justify-self-end"
                >
        ...
        <section id="about" data-nav-theme="light" className="sticky top-0 z-20 min-h-screen border-y border-[#DCDFE5] bg-white shadow-[0_-24px_60px_-20px_rgba(0,0,0,0.25)]">
          <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
            <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
              ...
              <Reveal delay={0.15} from="right">
                <p className="max-w-3xl text-lg leading-8 text-[#475569]">
                  {t('about.description')}
                </p>
              </Reveal>
            </div>

            <div className="mt-20 flex flex-col justify-between gap-6 border-t border-[#D4DAE3] pt-10 md:flex-row md:items-end">
              <Reveal>
              ...
              <Reveal delay={0.1} from="right">
                <a href="/ekibimiz" className="group inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.18em] text-[#9C7846]">
                      ...
                      <div className="aspect-square overflow-hidden bg-[#E5E2D8]">
                        <img
                          ...
                          onError={(e) => {
                            ...
                            e.currentTarget.parentElement.innerHTML = `<div class="flex h-full w-full items-center justify-center bg-[#E5E2D8] font-fraunces text-5xl text-[#B8946B]">${initials}</div>`;
                          }}
                        />
                      </div>
                      <div className="border-b border-[#D4DAE3] py-5">
                        <h3 className="text-xl font-semibold text-[#0A1428]">{member.name}</h3>
                        <p className="mt-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#B8946B]">
                          {title}
                        </p>
                      </div>
                    </a>
        ...
        <section id="practice-areas" data-nav-theme="light" className="sticky top-0 z-30 min-h-screen bg-[#F1EEE7] shadow-[0_-24px_60px_-20px_rgba(0,0,0,0.25)]">
          <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
            <div className="mb-12 border-b border-[#D4DAE3] pb-8">
              <SectionHeader
                eyebrow={t('practiceAreas.title')}
                title={t('practiceAreas.subtitle')}
                action={
                  <a href="/calisma-alanlari" className="group inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.18em] text-[#9C7846]">
          ...
          >
            {PRACTICE_AREAS.map((area, i) => {
              ...
              return (
                <article
                  key={area.id}
                  className="relative shrink-0 w-[88vw] max-w-[720px] h-[60vh] max-h-[520px] bg-white border border-[#D4DAE3] p-10 md:p-12 flex flex-col"
                >
                  {/* Numara + gradient aksenti */}
                  <div className="absolute top-0 left-0 h-1 w-full bg-gradient-to-r from-[#B8946B] to-transparent" />
                  <div className="flex items-baseline gap-4 mb-6">
                  ...
                  <h3 className="font-fraunces text-2xl md:text-3xl font-semibold leading-tight text-[#0A1428]">
                    {content.title}
                  </h3>

                  <p className="mt-4 text-[15px] leading-7 text-[#475569] line-clamp-4">
                    {content.lede}
                  </p>
                  ...
                  <ul className="mt-6 space-y-2 flex-1 overflow-hidden">
                    {content.services.slice(0, 3).map((s, j) => (
                      <li key={j} className="flex items-start gap-3 text-sm text-[#3a3a3a]">
                        <span className="font-mono text-xs text-[#8E96A5] mt-1">
                          {String(j + 1).padStart(2, '0')}
                        </span>
                    ...
                    {content.services.length > 3 && (
                      <li className="text-xs text-[#8E96A5] pl-7">
                  ...
                  <a
                    ...
                    className="group mt-6 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#0A1428]"
                  >
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-6 lg:px-8 pb-10">
      <div className="flex items-center gap-6">
        <div className="relative h-px flex-1 bg-[#D4DAE3]">
          <motion.div
            className="absolute left-0 top-0 h-full bg-[#B8946B]"
            style={{ width: widthPercent }}
          />
        </div>
        <span className="text-xs font-mono text-[#8E96A5] whitespace-nowrap">
          {String(current).padStart(2, '0')} / {String(total + 1).padStart(2, '0')}
        </span>
      </div>
      <p className="mt-3 text-center text-[10px] uppercase tracking-[0.3em] text-[#8E96A5]">
